#!/bin/sh
# Manage dotfile-owned Raycast preferences that live in the defaults domain.
# Typical flow:
#   ./scripts/raycast-defaults.sh status
#   ./scripts/raycast-defaults.sh --dry-run apply
#   ./scripts/raycast-defaults.sh apply
# Raycast reads these keys at launch, so restart Raycast after `apply`.
set -eu

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
DOTFILES_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
PROFILE_FILE="${RAYCAST_DEFAULTS_PROFILE:-$DOTFILES_DIR/.config/raycast/defaults.conf}"
# Override with a plist path (e.g. /tmp/x/com.raycast.macos.plist) to test in isolation.
DOMAIN="${RAYCAST_DEFAULTS_DOMAIN:-com.raycast.macos}"
DRY_RUN=0
PARSED_KEY=""
PARSED_TYPE=""
PARSED_VALUE=""
...
usage() {
  cat << USAGE
Usage: ./scripts/raycast-defaults.sh [--dry-run] <command> [path]
...
Commands:
  ...
  apply                   Write the profile with defaults(1); restart Raycast afterwards
  export-defaults [path]  Write current live values in profile format

Environment:
  RAYCAST_DEFAULTS_PROFILE  Profile file (default: .config/raycast/defaults.conf)
  RAYCAST_DEFAULTS_DOMAIN   defaults domain or plist path (default: com.raycast.macos)

Examples:
  ./scripts/raycast-defaults.sh status
  ./scripts/raycast-defaults.sh --dry-run apply
  ./scripts/raycast-defaults.sh export-defaults ./.config/raycast/defaults.conf
USAGE
}
...
parse_profile_line() {
  ...
  set -- $_line
  [ "$#" -eq 3 ] || die "Invalid line (expected <key> <type> <value>): $_raw_line"
  PARSED_KEY="$1"
  PARSED_TYPE="$2"
  PARSED_VALUE="$3"
...
validate_profile_syntax() {
  ...
  while IFS= read -r _raw_line || [ -n "$_raw_line" ]; do
    ...
    case "$_seen_keys" in
      *"|$PARSED_KEY|"*) die "Duplicate key in $(basename "$PROFILE_FILE"): $PARSED_KEY" ;;
    esac
...
read_live_value() {
  _key="$1"
  _type="$2"
  if ! _value="$(defaults read "$DOMAIN" "$_key" 2> /dev/null)"; then
    printf '<unset>'
    return 0
  fi
  if [ "$_type" = "bool" ]; then
...
print_status() {
  ...
  printf '\n'
  printf '%-36s %-7s %-34s %-34s %s\n' "key" "type" "current" "target" "state"
  printf '%-36s %-7s %-34s %-34s %s\n' "------------------------------------" "-------" \
    "----------------------------------" "----------------------------------" "-----"
  ...
  while IFS= read -r _raw_line || [ -n "$_raw_line" ]; do
    parse_profile_line "$_raw_line"
    [ -n "$PARSED_KEY" ] || continue
    _current_value="$(read_live_value "$PARSED_KEY" "$PARSED_TYPE")"
    _state="drift"
    ...
    printf '%-36s %-7s %-34s %-34s %s\n' "$PARSED_KEY" "$PARSED_TYPE" "$_current_value" "$PARSED_VALUE" "$_state"
  done < "$PROFILE_FILE"
...
apply_profile() {
  ...
  while IFS= read -r _raw_line || [ -n "$_raw_line" ]; do
    ...
    if [ "$DRY_RUN" -eq 1 ]; then
      printf 'defaults write %s %s %s %s\n' "$DOMAIN" "$PARSED_KEY" "$_flag" "$_write_value"
      continue
    fi
    defaults write "$DOMAIN" "$PARSED_KEY" "$_flag" "$_write_value"
    printf 'wrote: %s=%s\n' "$PARSED_KEY" "$_write_value"
  done < "$PROFILE_FILE"
  ...
  if [ "$DRY_RUN" -eq 1 ]; then
    ...
    printf 'Restart Raycast to load the new values.\n'
  fi
}
...
emit_live_profile() {
  ...
  require_command defaults
  validate_profile_syntax
  printf '# Snapshot captured from %s on %s.\n' "$DOMAIN" "$(date '+%Y-%m-%d %H:%M:%S %z')"
  printf '# Format: <key> <type> <value>   (type: string | bool | int)\n'
  while IFS= read -r _raw_line || [ -n "$_raw_line" ]; do
    parse_profile_line "$_raw_line"
    [ -n "$PARSED_KEY" ] || continue
    _current_value="$(read_live_value "$PARSED_KEY" "$PARSED_TYPE")"
    if [ "$_current_value" = "<unset>" ]; then
      printf '# %s is unset on this machine; keeping the tracked value.\n' "$PARSED_KEY"
      _current_value="$PARSED_VALUE"
    fi
    if [ "$PARSED_TYPE" = "bool" ]; then
      _current_value="$(bool_word "$_current_value")"
    fi
    printf '%-36s %-7s %s\n' "$PARSED_KEY" "$PARSED_TYPE" "$_current_value"
  done < "$PROFILE_FILE"
}
